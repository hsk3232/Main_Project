...
public class CsvSaveService {
	...
	public void postCsv(MultipartFile file, CustomUserDetails user) {
		...
		try (BufferedReader reader = new BufferedReader(
			...
			// >>> 여기서 Factory location_id 검사 추가
			Set<Long> factoryLocationIds = new HashSet<>();
			for (String[] r : allRows) {
			    String businessStep = getValue(colIdx, r, "business_step");
			    if ("Factory".equalsIgnoreCase(businessStep)) {
			        Long locId = parseLongSafe(getValue(colIdx, r, "location_id"));
			        if (locId != null) factoryLocationIds.add(locId);
			    }
			}

			if (!factoryLocationIds.contains(member.getLocation().getLocationId())) {
			    throw new BadRequestException(
			        "[업로드 실패] 회원의 location_id(" + member.getLocation().getLocationId()
				...
				try {
					...
					if (epcCode != null && !insertedEPCs.contains(epcCode)) {
						epcs.add(Epc.builder()
								.epcCode(epcCode).epcHeader(getValue(colIdx, dataRow, "epcHeader"))
