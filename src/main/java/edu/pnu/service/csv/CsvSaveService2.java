...
//	
//	// [실시간 진행률/상태 관리용 Map]
//    //  - uploadId 별로 진행률(%)과 상태값 저장
//    private final Map<String, Integer> progressMap = new ConcurrentHashMap<>();
//    private final Map<String, String> statusMap = new ConcurrentHashMap<>();
//    
//    
//    /**
//     * [1] 파일 임시 저장 후 uploadId 발급
//     *  - 파일을 임시 폴더/DB 등에 저장(여기선 예시로 폴더에 저장)
//     *  - 실제 운영환경에 맞춰 저장 로직 조정 가능
//     */
//    public String saveTempFile(MultipartFile file, CustomUserDetails user) {
//        String uploadId = UUID.randomUUID().toString();
//        try {
//            
//            String tempPath = "c:/MainProject/temp_csv/" + uploadId + ".csv";
...
//            statusMap.put(uploadId, "PENDING");
//        } catch (Exception e) {
//            progressMap.put(uploadId, 0);
//            statusMap.put(uploadId, "FAILED");
...
//    
//    /**
//     * [2] 비동기 배치 저장(진행률/상태 관리)
//     *  - 업로드ID별 임시 파일을 읽어서 파싱/DB저장
//     *  - 진행률/상태는 Map으로 관리(Polling API 연동)
//     *  - @Async: 반드시 @EnableAsync 선언 필요!
//     */
//    @Async
//	public void postCsv(String uploadId, CustomUserDetails user) {
//    	
//    	// --- 1. 임시 파일 경로 구성 ---
//        String tempPath = "c:/MainProject/temp_csv/" + uploadId + ".csv";
...
//			
//			
//            // [5] DB에 이미 있는 ID 한 번에 모두 조회 (개별 existsById 대신 성능↑)
...
//
//            // [6] 저장/중복 방지용 Set, 실제 저장할 객체 리스트 준비
...
//
//            int batchSize = 5000; // 너무 크면 메모리 부족 가능, 2000~5000 추천!
//            int totalRows = allRows.size();
//            int savedCount = 0;
//
//            // [7] 한 행씩 객체 생성 + 중복 체크 + 배치 저장
//            for (int i = 0; i < allRows.size(); i++) {
//                String[] dataRow = allRows.get(i);
//                int rowNum = i + 2; // CSV 파일에서 헤더+1이 실제 행 번호
//                final String errorRow = "[row " + rowNum + "]"; // 디버그용
//
//                try {
//                	// [A] Location 엔티티 생성 (중복 저장 방지)
